...
var SpinningWheel = 
{
	...
	{
		 // alert('evento');
		// console.log('handleEvent:0');
		// console.log('e.currentTarget.id=>'+e.currentTarget.id);
		 
		/* if(e.currentTarget.id==null || e.currentTarget.id=='undefined')
		 {
		 	//alert('paso');
		 	console.log('paso')
		 	var obj=document.getElementbyID('sw-frame');
		...
		{
			//console.log('handleEvent:1');
			this.lockScreen(e);
			if (e.currentTarget.id == 'sw-cancel' || e.currentTarget.id == 'sw-done') 
			{
			//	console.log('handleEvent:2');
				this.tapDown(e);
			} 
			else
			 if (e.currentTarget.id == 'sw-frame') 
			 {
			 //	console.log('handleEvent:3');
				this.scrollStart(e);
			}
		} 
		else
		 if (e.type == 'touchmove') 
		{
			//console.log('handleEvent:4');
			this.lockScreen(e);
			
			if (e.currentTarget.id == 'sw-cancel' || e.currentTarget.id == 'sw-done') 
			{
				//console.log('handleEvent:5');
				this.tapCancel(e);
			} 
			else 
			if (e.currentTarget.id == 'sw-frame' ) 
			{
				//console.log('handleEvent:6');
				this.scrollMove(e);
			}
		}
		 else
		  if (e.type == 'touchend') 
		 {
			if (e.currentTarget.id == 'sw-cancel' || e.currentTarget.id == 'sw-done') 
			{
				//console.log('handleEvent:7');
				this.tapUp(e);
			} 
			else
			 if (e.currentTarget.id == 'sw-frame') 
			 {
			 	//console.log('handleEvent:8');
				this.scrollEnd(e);
			}
		} 
		else
		...
		{
			//console.log('handleEvent:9');
			if (e.target.id == 'sw-wrapper') 
			{
				//console.log('handleEvent:10');
				this.destroy();
			} 
			else 
			{
				//console.log('handleEvent:11');
				this.backWithinBoundaries(e);
			}
		} 
		else if (e.type == 'orientationchange') 
		{
			//console.log('handleEvent:12');
			this.onOrientationChange(e);
		} 
		else 
		if (e.type == 'scroll') 
		{
			//console.log('handleEvent:13');
			this.onScroll(e);
		}
	...
	onOrientationChange: function (e) 
	{
		...
		//this.swWrapper.style.top = window.innerHeight + window.pageYOffset + 'px';
		 //this.swWrapper.style.top = '307px';

		this.swWrapper.style.top = _top;  
	...
	create: function () 
	{
		...
		div.style.top =  _top;		// Place the SW down the actual viewing screen
		...
		{	
			...
			{
				out += '<li id=_'+i+'>' + this.slotData[l].values[i] + '<' + '/li>';
			}
			ul.innerHTML = out;
